...
public class SPARQLTest {
    ...
    public void constructAllTest() throws Exception {
        ...
        URI inputWriteContext = input.addNewDataGraph("iii");
        ...
        try {
            ...
            String baseURI = "";
            connection.add(inputStream, baseURI, RDFFormat.TURTLE, inputWriteContext);

            // some triples has been loaded
            assertTrue(connection.size(inputWriteContext) > 0);
            // run
            env.run(trans);
            connection2 = output.getConnection();
            // verify result
            assertTrue(connection.size(inputWriteContext) == connection2.size(RDFHelper.getGraphsURIArray(output)));
        } finally {
            if (connection != null) {
                try {
                    connection.close();
