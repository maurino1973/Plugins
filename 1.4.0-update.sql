...
/*!40000 ALTER TABLE `dpu_template` ENABLE KEYS */  WHERE `jar_name` LIKE '%.jar';
UNLOCK TABLES  WHERE `jar_name` LIKE '%.jar';
/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */  WHERE `jar_name` LIKE '%.jar';

/*!40101 SET SQL_MODE=@OLD_SQL_MODE */  WHERE `jar_name` LIKE '%.jar';
/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */  WHERE `jar_name` LIKE '%.jar';
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */  WHERE `jar_name` LIKE '%.jar';
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */  WHERE `jar_name` LIKE '%.jar';
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */  WHERE `jar_name` LIKE '%.jar';
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */  WHERE `jar_name` LIKE '%.jar';
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */  WHERE `jar_name` LIKE '%.jar';

-- Dump completed on 2014-09-11 15:41:45
DELETE FROM `properties` WHERE `key` = 'UV.Plugins.version'  WHERE `jar_name` LIKE '%.jar';
INSERT INTO `properties` SET `key` = 'UV.Plugins.version', `value` = '001.004.000'  WHERE `jar_name` LIKE '%.jar';
